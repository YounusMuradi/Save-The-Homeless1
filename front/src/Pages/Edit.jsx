...
const Edit = () => {
  ...
  const fetchData = async()=>{
    try{
      const result = await axios.get("http://127.0.0.1:8000/api/donors/"+id);
      // console.log(result.data.results);
      setDonarField(result.data.results);
  ...
  const onSubmitchange = async (e) =>{
    e.preventDefault();
    try{
      await axios.put("http://127.0.0.1:8000/api/donoerupdate/"+id,donarField);
      navigate('/donar');
    }catch(err){
      console.log("something went wrong");
    }
